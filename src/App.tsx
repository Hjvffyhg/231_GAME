...
export default function App() {
  const [view, setView] = useState<
    "boot" | "menu" | "game" | "report" | "modes" | "shop" | "codex"
  >("boot");
      ...
      <AnimatePresence mode="wait">
        {view === "boot" ? (
          ...
          <motion.div
            ...
            className="absolute inset-0 z-50 bg-[#0A0F1F] origin-center"
          >
            <MainMenu
              onStartGame={() => {
                ...
                {isGameOver && (
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        className={cn(
                          "absolute inset-0 border backdrop-blur-sm transition-all",
                          isVictory
                      ...
                      <div
                        className={cn(
                          "absolute left-0 top-0 bottom-0 w-2 shadow-[0_0_15px]",
                          isVictory
                      ...
                      <div className="relative z-10 w-full flex flex-col items-center text-center">
                        {isVictory ? (
                          <>
                            <div className="inline-block bg-emerald-500/20 border border-emerald-500/50 text-emerald-400 px-4 py-2 mb-6 text-xl font-bold uppercase tracking-widest shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                              HIVE NETWORK DESTROYED
                            </div>
                            <p className="mb-2 text-emerald-100 font-mono tracking-wide">
                              ENEMY FLAGSHIP ELIMINATED
                            </p>
                            <p className="mb-8 opacity-70 font-mono text-sm">
                              EARTH DEFENSE COMMAND CONFIRMS VICTORY
                            </p>
                          </>
                        ) : (
                          <>
                            <div className="inline-block bg-rose-500/20 border border-rose-500/50 text-rose-400 px-4 py-2 mb-6 text-xl font-bold uppercase tracking-widest shadow-[0_0_15px_rgba(244,63,94,0.3)]">
                              MISSION FAILED
                            </div>
                            <p className="mb-6 text-slate-300 font-mono tracking-wide">
                              SHIP DESTROYED{" "}
                        ...
                        >
                          ...
                          <div
                            className={cn(
                              "text-5xl font-black tracking-wider drop-shadow-[0_0_10px_rgba(255,255,255,0.2)] text-center",
                              isVictory ? "text-emerald-100" : "text-white",
                            )}
                          >
                            {finalScore}
                          </div>
                        ...
                        <div className="flex flex-col sm:flex-row justify-center gap-6 w-full">
                          ...
                          >
                            <div
                              className={cn(
                                "absolute inset-0 translate-x-[-100%] group-hover/btn:translate-x-[100%] transition-transform duration-1000",
                                isVictory
                                  ? "bg-gradient-to-r from-emerald-500/0 via-emerald-500/20 to-emerald-500/0"
                                  : "bg-gradient-to-r from-rose-500/0 via-rose-500/20 to-rose-500/0",
                              )}
                            ></div>
                            <div
                              className={cn(
                                ...
                                isVictory
                                  ? "bg-emerald-700 group-hover/btn:bg-emerald-400"
                                  : "bg-rose-700 group-hover/btn:bg-rose-400",
                              )}
                            ></div>
                            <span
                              className={cn(
                                "relative z-10 font-mono text-sm md:text-base font-bold tracking-[0.2em] transition-colors uppercase drop-shadow-lg flex items-center gap-3",
                                isVictory
                                  ? "text-emerald-100 group-hover/btn:text-white"
                                  : "text-rose-100 group-hover/btn:text-white",
                              )}
                            >
