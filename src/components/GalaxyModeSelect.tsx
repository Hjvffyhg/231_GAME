    ...
    <div className="absolute inset-0 bg-[#020617] flex flex-col p-4 pt-12 md:p-8 font-sans h-full overflow-y-auto custom-scrollbar">
      ...
      <div className="relative z-10 text-center mb-8 md:mb-12 flex flex-col items-center">
        <div className="flex items-center gap-3 justify-center mb-2">
            <Crosshair className="text-[#00D9FF] animate-[spin_10s_linear_infinite]" size={28} />
            ...
            <Crosshair className="text-[#00D9FF] animate-[spin_10s_linear_infinite_reverse]" size={28} />
        </div>
        <div className="bg-[#0d1428]/80 border border-[#00D9FF]/40/50 px-6 py-2 mt-2 inline-block backdrop-blur-sm" style={{ clipPath: 'polygon(5% 0, 95% 0, 100% 100%, 0 100%)' }}>
            <p className="text-xs md:text-sm text-[#00D9FF] font-mono tracking-widest uppercase">Max Historical Combat Score: <span className="text-white font-bold">{highScore.toLocaleString()}</span></p>
        </div>
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto w-full pb-10">
        {MODES.map((mode, idx) => {
          ...
          return (
            ...
            >
              {/* Corner Deco */}
              {unlocked && <div className="absolute top-0 right-0 w-8 h-8 bg-[#00D9FF]/20 transition-colors group-hover:bg-[#00D9FF]/40" style={{ clipPath: 'polygon(100% 0, 0 0, 100% 100%)' }}></div>}
              {!unlocked && <div className="absolute top-0 right-0 w-8 h-8 bg-[#EF4444]/10" style={{ clipPath: 'polygon(100% 0, 0 0, 100% 100%)' }}></div>}

              {!unlocked && (
                <div className="absolute inset-0 bg-[#0A0F1F]/80 backdrop-blur-[1px] z-20 flex flex-col items-center justify-center">
                  <div className="border border-[#EF4444]/50 bg-[#EF4444]/20/50 p-4 flex flex-col items-center" style={{ clipPath: 'polygon(20% 0, 80% 0, 100% 20%, 100% 80%, 80% 100%, 20% 100%, 0 80%, 0 20%)' }}>
              ...
              <div className="flex justify-between items-start mb-5">
                <div className={`w-10 h-10 border flex items-center justify-center transition-colors ${unlocked ? 'border-[#00D9FF]/50 bg-[#00D9FF]/20/50 group-hover:bg-[#00D9FF]/20' : 'border-[#151f3d] bg-[#0A0F1F]'}`} style={{ clipPath: 'polygon(30% 0, 70% 0, 100% 30%, 100% 70%, 70% 100%, 30% 100%, 0 70%, 0 30%)' }}>
                  <Globe2 size={20} className={unlocked ? "text-[#00D9FF]" : "text-slate-600"} />
                </div>
                <div className="text-right">
                  <div className={`text-[10px] font-mono font-bold tracking-widest ${unlocked ? 'text-[#00D9FF]' : 'text-slate-600'}`}>TYPE {mode.level}</div>
              ...
              <p className="text-xs text-slate-400 flex-grow leading-relaxed font-mono border-l-2 border-[#151f3d] pl-3">{mode.desc}</p>
              ...
              <div className="mt-6 pt-4 border-t border-[#151f3d]/50 flex flex-col gap-2">
                ...
                {unlocked && (
                    <div className="w-full text-center mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <span className="text-[10px] font-mono font-bold text-[#00D9FF] bg-[#00D9FF]/20/50 px-4 py-1 border border-[#00D9FF]/30 tracking-widest uppercase">Initiate Jump</span>
                    </div>
                )}
              </div>
            </motion.div>
