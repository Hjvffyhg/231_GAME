...
export function GameCanvas({ gameKey, onGameOver, onReturnMenu, civilizationLevel = 0 }: GameCanvasProps) {
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (state.keys['c'] && state.player.dashCooldown <= 0 && state.player.stamina >= 15) {
        ...
        for (let j = i + 1; j < state.asteroids.length; j++) {
            ...
            if (overlap > 0 && dist > 0) {
                ...
                if (relVel < 0) {
                    ...
                    const impulse = -(1 + restitution) * relVel / (1/a.radius + 1/b.radius);
      ...
      state.enemyBullets.forEach((b: any) => {
        ...
        if (!b.trail) b.trail = [];
        b.trail.unshift({x: b.x, y: b.y});
        if (b.trail.length > (b.bulletType?.startsWith('boss') ? 40 : 15)) b.trail.pop();
      ...
      if (wpn1Ref.current && wpn2Ref.current) {
          ...
          wpn1Ref.current.innerHTML = `<span class="z-10 text-[10px] md:text-[13px] font-black tracking-widest px-1.5 py-0.5 border-t border-b border-rose-500/50 bg-rose-950/80 drop-shadow-lg text-rose-100 uppercase">KIN</span><div class="absolute bottom-1 md:bottom-2 right-3 md:right-4 text-slate-400 text-[8px] z-10 font-bold drop-shadow-md">1</div>`;
      ...
      if (dshRef.current) {
          ...
          dshRef.current.style.clipPath = 'polygon(15% 0%, 100% 0%, 85% 100%, 0% 100%)';
    ...
    const render = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, state: any, cameraX: number, cameraY: number) => {
        ...
        state.asteroids.forEach((a: any) => {
            ...
            for(let i=0; i<3; i++) {
                ...
                const cr = a.radius * (0.15 + ((Math.sin(a.id * 5 + i) + 1)/2) * 0.15);
        ...
        state.damageNumbers.forEach((d: any) => {
           ...
           const text = d.isCrit ? `${Math.ceil(d.amount)} CRIT` : Math.ceil(d.amount).toString();
           
           ctx.font = `bold ${d.isCrit ? '26px' : '20px'} "Inter", sans-serif`;
           ctx.textAlign = 'center';
           ctx.textBaseline = 'middle';
           
           ctx.strokeStyle = '#000';
           ctx.lineWidth = d.isCrit ? 5 : 3;
           ctx.lineJoin = 'round';
        ...
        state.enemyBullets.forEach((b: any) => {
           ...
           if (b.bulletType?.startsWith('boss')) {
               shape = 'image';
               if (b.bulletType === 'boss_slow') { w = 64; h = 64; blur = 30; color = '#dc2626'; isLightning = true; }
               else if (b.bulletType === 'boss_burst') { w = 32; h = 32; blur = 20; color = '#f59e0b'; }
               else if (b.bulletType === 'boss_spiral') { w = 48; h = 48; blur = 25; color = '#ec4899'; isLightning = true; }
               else if (b.bulletType === 'boss_sweep') { w = 24; h = 24; blur = 15; color = '#8b5cf6'; }
               else if (b.bulletType === 'boss_focused') { w = 56; h = 56; blur = 30; color = '#ef4444'; isLightning = true; }
               else if (b.bulletType === 'boss_snipe') { w = 40; h = 120; blur = 40; color = '#f87171'; isLightning = true; } // long beam
               ...
               shape = 'circle';
               if (b.bulletType === 'grunt_snipe') { size = 5; blur = 15; color = '#fcd34d'; isLightning = true; }
               ...
               shape = 'circle'; size = 6; blur = 12; color = '#c084fc'; isLightning = true;
           } else {
               // Fallback logic
               shape = b.isBig ? 'image' : 'circle';
               if (b.isBig) { w = 48; h = 48; blur = 25; color = '#ec4899'; isLightning = true; }
               else { size = 4; blur = 10; color = '#34d399'; }
           }
           ...
           if (isLightning && b.trail && b.trail.length > 2) {
               ...
               for (let i = 0; i < b.trail.length; i++) {
                   ...
                   // Add jaggedness
                   const amplitude = i === 0 || i === b.trail.length - 1 ? 0 : (Math.random() - 0.5) * (w * 0.5);
               ...
               for (let i = 0; i < b.trail.length; i++) {
                   ...
                   const offX = len > 0 ? (perpX/len) : 0;
                   const offY = len > 0 ? (perpY/len) : 0;
                   const amplitude = i === 0 || i === b.trail.length - 1 ? 0 : (Math.random() - 0.5) * (w * 0.2);
                   ctx.lineTo(pt.x + offX * amplitude, pt.y + offY * amplitude);
               }
