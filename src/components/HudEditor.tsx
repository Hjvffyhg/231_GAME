...
export function HudEditor({ onExit }: { onExit: () => void }) {
  ...
  return (
    <div
      className="absolute inset-0 bg-[#0A0F1F]/90 z-50 overflow-hidden select-none font-sans backdrop-blur-sm"
      ref={containerRef}
    ...
    >
      ...
      {renderElement(
        "wpnBtn",
        <div
          className="w-16 h-16 bg-[#EF4444]/20/60 border-2 border-[#EF4444] flex items-center justify-center text-[#EF4444] font-mono font-bold text-sm shadow-[0_0_15px_rgba(244,63,94,0.4)] backdrop-blur-md"
          style={{
            clipPath:
      ...
      {renderElement(
        "shdBtn",
        <div
          className="w-16 h-16 bg-blue-950/60 border-2 border-blue-400 flex items-center justify-center text-blue-300 font-mono font-bold text-sm shadow-[0_0_15px_rgba(96,165,250,0.4)] backdrop-blur-md"
          style={{
            clipPath:
              "polygon(30% 0%, 70% 0%, 100% 30%, 100% 70%, 70% 100%, 30% 100%, 0% 70%, 0% 30%)",
          }}
      ...
      {renderElement(
        "dshBtn",
        <div
          className="w-14 h-14 bg-[#151f3d]/60 border border-slate-300 flex items-center justify-center text-slate-200 font-mono font-bold text-xs shadow-[0_0_10px_rgba(255,255,255,0.2)] backdrop-blur-md"
      ...
      {renderElement(
        "hpBar",
        <div
          className="flex flex-col gap-1 w-56 bg-[#0d1428]/80 p-3 border border-[#00D9FF]/40/50 backdrop-blur-md shadow-[0_0_20px_rgba(6,182,212,0.15)] relative overflow-hidden"
        ...
        >
          ...
          <div className="w-full h-1.5 bg-[#151f3d]">
            <div className="w-full h-full bg-[#00D9FF] shadow-[0_0_8px_#00D9FF]"></div>
          </div>
          ...
          <div className="w-full h-1 bg-[#151f3d]">
            <div className="w-full h-full bg-[#EF4444] shadow-[0_0_8px_#EF4444]"></div>
          </div>
        </div>,
        "HULL_TELEMETRY",
      )}
        ...
        >
          ...
          <div className="bg-[#00D9FF]/20/40 px-5 py-3 flex justify-between items-center border-b border-[#00D9FF]/30">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono font-bold text-sm text-cyan-100 tracking-widest uppercase">
                Node Config
              </span>
            </div>
            <span className="text-[10px] text-[#00D9FF] bg-[#00D9FF]/20 px-2 py-1 font-mono border border-cyan-800/50">
              {selectedId}
            </span>
          </div>
          ...
          <div className="p-6 flex gap-6">
            ...
            <div className="flex-1 flex flex-col gap-6">
              <div>
                <div className="flex justify-between text-[10px] font-mono font-bold text-[#00D9FF] mb-2 uppercase tracking-wider">
                  ...
                  <span className="text-white">
                    {(layout[selectedId].scale * 100).toFixed(0)}%
                  </span>
                </div>
                <input
                  type="range"
              ...
              <div>
                <div className="flex justify-between text-[10px] font-mono font-bold text-[#00D9FF] mb-2 uppercase tracking-wider">
                  ...
                  <span className="text-white">
                    {(layout[selectedId].opacity * 100).toFixed(0)}%
                  </span>
                </div>
                <input
                  type="range"
            ...
            <div className="w-24 grid grid-cols-3 grid-rows-3 gap-1 content-center">
              <button
                onClick={() => nudge(0, -1)}
                className="col-start-2 bg-[#151f3d] hover:bg-[#00D9FF]/30/50 border border-[#1e2b52] hover:border-[#00D9FF] flex items-center justify-center active:scale-95 transition-all text-[#00D9FF] hover:text-[#5ce5ff]"
              >
                <ArrowUp size={14} />
              </button>
              <button
                onClick={() => nudge(-1, 0)}
                className="row-start-2 col-start-1 bg-[#151f3d] hover:bg-[#00D9FF]/30/50 border border-[#1e2b52] hover:border-[#00D9FF] flex items-center justify-center active:scale-95 transition-all text-[#00D9FF] hover:text-[#5ce5ff]"
              >
                <ArrowLeft size={14} />
              </button>
              ...
              <button
                onClick={() => nudge(0, 1)}
                className="row-start-3 col-start-2 bg-[#151f3d] hover:bg-[#00D9FF]/30/50 border border-[#1e2b52] hover:border-[#00D9FF] flex items-center justify-center active:scale-95 transition-all text-[#00D9FF] hover:text-[#5ce5ff]"
              >
                <ArrowDown size={14} />
              </button>
              <button
                onClick={() => nudge(1, 0)}
                className="row-start-2 col-start-3 bg-[#151f3d] hover:bg-[#00D9FF]/30/50 border border-[#1e2b52] hover:border-[#00D9FF] flex items-center justify-center active:scale-95 transition-all text-[#00D9FF] hover:text-[#5ce5ff]"
              >
                <ArrowRight size={14} />
              </button>
            </div>
          ...
          <div className="flex border-t border-[#00D9FF]/40/50 font-mono font-bold text-xs text-center bg-[#0d1428]/50">
            <button
              onClick={() => {
                setLayout((prev) => ({
                  ...prev,
                  [selectedId]: DEFAULT_LAYOUT[selectedId],
                }));
              }}
