...
export const Mermaid = ({ chart }: { chart: string }) => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      mermaid
        .render(
          `mermaid-chart-${Math.random().toString(36).substring(2, 9)}`,
          chart,
        )
        .then(({ svg }) => {
          if (containerRef.current) {
    ...
    return (
      <div className="flex justify-center items-center py-4 w-full">
        <div className="text-amber-400 border border-amber-500/30 bg-amber-950/20 p-4 rounded text-center text-sm font-mono max-w-md shadow-[0_0_15px_rgba(251,191,36,0.1)]">
          <span className="block font-bold mb-2 uppercase">
            Module Load Interrupted
          </span>
          The dynamic visualizer chunk failed to load. A visualizer dependency
  ...
  return (
    <div
      ref={containerRef}
      className="flex justify-center items-center py-4 w-full overflow-x-auto"
    />
  );
};
