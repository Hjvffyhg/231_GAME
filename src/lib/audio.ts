export class SoundManager {
  ...
  playBGM(intensity: 'low' | 'med' | 'high') {
     ...
     const scheduleBGM = () => {
         while (nextNoteTime < this.ctx!.currentTime + 0.1) {
             ...
             if (step % 2 === 0 && (this.bgmIntensity === 'med' || this.bgmIntensity === 'high')) {
             ...
             const bassNotes = [55.00, 55.00, 55.00, 65.41, 55.00, 55.00, 61.74, 58.27]; // A1, A1, A1, C2, A1, A1, B1, Bb1
             const hz = bassNotes[step % 8];
             
             const bass = this.ctx!.createOscillator();
             const bassVol = this.ctx!.createGain();
             bass.type = 'sawtooth';
             bass.frequency.setValueAtTime(this.bgmIntensity === 'high' ? hz * 2 : hz, nextNoteTime);
             bassVol.gain.setValueAtTime(0.3, nextNoteTime);
             bassVol.gain.exponentialRampToValueAtTime(0.01, nextNoteTime + eighthNote * 0.9);
  ...
  playAlgoSelect(algo: string) {
    ...
    if (algo === 'FCFS') {
        ...
        gain2.gain.exponentialRampToValueAtTime(0.01, this.ctx.currentTime + 0.13);
