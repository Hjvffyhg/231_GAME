...
function loadImage(src: string): Promise<HTMLImageElement | null> {
  return new Promise((resolve) => {
    ...
    img.onerror = () => {
      console.warn(`[VoidFleet] Missing asset: ${src}`);
      resolve(null);
    };
    img.src = src;
  });
...
export async function loadAllProjectiles(): Promise<
  Record<ProjectileName, HTMLImageElement | null>
> {
  ...
  constructor(sprites: ShipSprites, cfg: ShipConfig, scale = 3) {
    ...
    this.weapons =
      cfg.weapons != null
        ...
        : null;
    this.destruction = new AnimSprite(
      sprites.Destruction,
      s,
      s,
      cfg.destruction,
